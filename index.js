...
function detectText (imageURI) {
  let text;

  console.log(`Looking for text in image ${imageURI}`);
  return vision.textDetection({ source: { imageUri: `${imageURI}` } })
    .then(([detections]) => {
    ...
    .then(([detection]) => {
      ...
      const tasks = config.TO_LANG.map((lang) => {
        ...
        //translateText(messageData).then(messageData => {messageData)/;
...
function translateText(payload) {
  ...
  return Promise.resolve()
    ...
    .then(([translation]) => {
      ...
      console.log("Translated messageData"+JSON.stringify(messageData));
      return messageData;
    });
}
